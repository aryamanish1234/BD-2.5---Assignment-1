...
app.get('/products/filter/ram', (req, res) => {
  const ramValue = parseInt(req.query.ram);
  const filteredProducts = products.filter(product => product.ram === ramValue);
  res.json({ products: filteredProducts });
});

// Endpoint 5: Filter products by ROM
app.get('/products/filter/rom', (req, res) => {
  const romValue = parseInt(req.query.rom);
  const filteredProducts = products.filter(product => product.rom === romValue);
  res.json({ products: filteredProducts });
});

// Endpoint 6: Filter products by Brand
app.get('/products/filter/brand', (req, res) => {
  const brandValue = req.query.brand.toLowerCase();
  const filteredProducts = products.filter(product => product.brand.toLowerCase() === brandValue);
  res.json({ products: filteredProducts });
});

// Endpoint 7: Filter products by OS
app.get('/products/filter/os', (req, res) => {
  const osValue = req.query.os.toLowerCase();
  const filteredProducts = products.filter(product => product.os.toLowerCase() === osValue);
  res.json({ products: filteredProducts });
});

// Endpoint 8: Filter products by Price
app.get('/products/filter/price', (req, res) => {
  const priceValue = parseInt(req.query.price);
  const filteredProducts = products.filter(product => product.price <= priceValue);
  res.json({ products: filteredProducts });
});
